...
from app.models import er_lab as _er_lab  # noqa: F401
from app.models import er_lab_question as _er_lab_question  # noqa: F401
from app.models import er_lab_session as _er_lab_session  # noqa: F401
...
class ErdTutorConversation(Base):
    ...
    __table_args__ = (
        Index("ix_erd_tutor_conv_lab", "user_id", "er_lab_question_id", "session_id"),
        Index("ix_erd_tutor_conv_standalone", "user_id", "er_diagram_question_id"),
    )
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    er_lab_question_id = Column(Integer, ForeignKey("er_lab_questions.id"), nullable=True)
    er_diagram_question_id = Column(Integer, ForeignKey("er_diagram_questions.id"), nullable=True)
    session_id = Column(Integer, ForeignKey("er_lab_sessions.id"), nullable=True)
    context_type = Column(String(20), nullable=False)  # "lab" | "standalone"
    # state snapshot (cv_*)
