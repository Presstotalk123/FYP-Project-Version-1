...
import { IconArrowDown, IconArrowUp, IconX, IconDatabase } from '@tabler/icons-react';
...
export function LabItemList({ items, onRemove, onMove }: {
  ...
  if (items.length === 0) return <Text c="dimmed" ta="center" py="md">No items yet. Add from the pool or add a shared-DB section.</Text>;
  return (
    <Stack gap={8}>
      {items.map((it, i) => (
        <Paper key={it.id} withBorder p="sm" radius="md">
          <Group gap="sm" wrap="nowrap">
            ...
            <Text style={{ flex: 1 }}>{it.kind === 'sqllab' ? <><IconDatabase size={14} /> Shared-DB section</> : it.title}</Text>
